...
- `abandoned_cart_scan` (commerce plugin; `packages/commerce/src/personalization/abandoned-cart-job.ts`) — scans `behavior_events` for threads with purchase intent (`add_to_cart` / `checkout_start`) but no `purchase`, idle over an hour; records `abandoned_carts` rows (deduped), emits `cart_abandoned` audit events, and dispatches to `COMMERCE_RECOVERY_WEBHOOK` when configured. See [the commerce docs](https://github.com/felix-run/web/blob/main/commerce/docs/index.md).
- `geo_monitor_tick` (commerce plugin; `packages/commerce/src/geo/monitor-job.ts`) — replays each tenant's tracked shopping queries (`geo_queries`) through a generative engine and records brand presence/rank/competitors into `geo_observations`. Tuned via the `GEO_MONITOR` env JSON. See [the commerce docs](https://github.com/felix-run/web/blob/main/commerce/docs/index.md).
