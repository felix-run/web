...
- **reflect** — single-agent. Wraps a react loop with a verifier model that scores each final response against `spec.reflect.criteria`; below threshold the critique is appended as a synthetic user turn and react replays up to `spec.reflect.max_iterations`. See [internals/patterns.md](/internals/patterns/#reflect).
...
The `id` is a **logical** id resolved through `MODEL_ROUTES` (a JSON map in env vars) to `{ provider, model }`. See [deploy.md](/guide/deploy/) for examples.
...
List of tool names registered with the `ToolProvider`. The core built-ins are `calculator`, `list_skills`, `activate_skill`, `deactivate_skill`, and the scheduling set (`schedule_task`, `list_scheduled_tasks`, `scheduled_task_runs`, `cancel_scheduled_task` — see below). Skills can fold additional tool names into this list at build time.
...
Scheduled runs are [unattended](/internals/governance/#approvals), so approval-gated tools fail closed inside them regardless of what gets scheduled. See [spec.memory](#specmemory) for how a task can leave itself notes across firings.
...
Felix passes the request's `threadId` as `session` so a multi-turn conversation reuses the same sandbox DO and filesystem state persists across turns. See [`examples/sandbox-worker/`](https://github.com/felix-run/web/blob/main/packages/examples/sandbox-worker/) for the reference adapter.
...
See [`examples/browser-worker/`](https://github.com/felix-run/web/blob/main/packages/examples/browser-worker/) for the reference adapter.
...
On an [unattended run](/internals/governance/#approvals) — a cron tick, a replay — an addendum forbids preference/intent facts about any person entirely: no human spoke, so anything preference-shaped is the model narrating its own behavior, and storing it would let an automated run quietly rewrite what the agent believes about someone who was never there.
...
Absolute ceilings ([src/limits/models.ts](https://github.com/felix-run/web/blob/main/packages/src/limits/models.ts)):
...
`pii` runs four regex patterns (email, SSN, US phone, credit card) with SHA-256 fingerprints written to audit (never the raw value). `pii` is currently the only accepted provider — **any unknown provider name is rejected at parse time** (an unregistered provider would otherwise be silently skipped, disabling filtering while appearing protected), and `bedrock` is explicitly rejected until an AI Gateway content-policy hook lands. Omitting `targets` scans **both** input and output (the default is `[input, output]`, not `[]`). See [internals/governance.md](/internals/governance/).
