...
All routes return JSON. Rate limited at 100 req/60s per tenant.
...
The server computes `next_run_at` from the schedule and persists the row. `schedule: ""` makes the job on-demand only (it will never be returned by the cron sweep). Schedules use the standard 5-field cron syntax — see [deploy.md](/guide/deploy/) for the supported syntax.
...
A manual run is **attended** — it happens inside your request, with you waiting on it — so approval-gated tools behave exactly as they do in a chat turn. The cron sweep is unattended and fails those closed instead (see [governance](/internals/governance/#approvals)). That difference is deliberate: it lets an operator manually drive a job whose tools need a human, without that authorization silently carrying over to the unsupervised schedule.
...
{
  "manifests": [
    { "name": "quick", "active_version": 3, "updated_at": 1747142400000 },
...
```json
{
  ...
  "source": "tenant_d1",
  "version": 3,
  "manifest": { "apiVersion": "orchestrator/v1", "kind": "Agent", "metadata": { "name": "quick", "version": "2.0.0", "description": "tenant-customized", "tags": [] }, "spec": { /* ... */ } }
}
```
...
curl -s -X POST -H "Authorization: Bearer $JWT" \
  -H "Content-Type: application/json" \
  $BASE_URL/manifests/quick \
  -d '{
    "manifest": {
      "apiVersion": "orchestrator/v1",
      "kind": "Agent",
      "metadata": { "name": "quick", "version": "2.0.0", "description": "tenant-customized" },
...
curl -s -X POST -H "Authorization: Bearer $JWT" \
  -H "Content-Type: application/json" \
  $BASE_URL/manifests/quick/activate \
  -d '{"version": 2}' | jq
```

```json
{ "name": "quick", "active_version": 2, "updated_at": 1747250000000 }
```
...
curl -s -X POST ".../manifests/quick?activate=false" -d '{"manifest": …}'   # → version 3

# 2. Eval version 3 (pin it — the runner records the version on the run).
curl -s -X POST ".../eval/datasets/golden/run" \
  -d '{"candidate_manifest":"quick","candidate_version":3}'   # → run_id

# 3. Activate 3, gated on that passing run.
curl -s -X POST ".../manifests/quick/activate" \
...
curl -s -X POST -H "Authorization: Bearer $JWT" \
  -H "Content-Type: application/json" \
  $BASE_URL/manifests/quick/canary \
...
curl -s -X POST -H "Authorization: Bearer $JWT" \
  -H "Content-Type: application/json" \
  $BASE_URL/manifests/quick/rollback \
