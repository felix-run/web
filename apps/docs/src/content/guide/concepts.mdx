...
Prometheus metrics at `GET /metrics`, counters prefixed `orchestrator_*` (a deliberate hold-over so
existing dashboards keep working). Audit rows are buffered by the process that emits them and flushed
...
1. **Middleware** — body limit (1 MiB), then per-tenant rate limit, then authentication.
