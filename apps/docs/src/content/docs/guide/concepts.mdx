...
7. **Sandboxes** — every `sandboxes[]` entry becomes a tool whose executor is a `SandboxExecutor` targeting a worker-local Fetcher. See [manifest-reference.md#specsandboxes](/guide/manifest-reference/#specsandboxes) and [`examples/sandbox-worker/`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/examples/sandbox-worker/).
8. **Browser tools** — every `browser_tools[]` entry becomes a tool whose executor is a `BrowserExecutor` targeting a worker-local Fetcher wrapping `@cloudflare/puppeteer`. See [manifest-reference.md#specbrowser_tools](/guide/manifest-reference/#specbrowser_tools) and [`examples/browser-worker/`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/examples/browser-worker/).
