...
See [manifest-reference.md#speccontainers](/guide/manifest-reference/#speccontainers) for the manifest field reference and [`examples/python-sandbox/`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/examples/python-sandbox/) for an end-to-end sample (a 30-line mock-gateway Worker plus the manifest that calls it).
...
The signed bundle distributed via R2 is documented in [internals/governance.md](/internals/governance/). The minimal flow:
