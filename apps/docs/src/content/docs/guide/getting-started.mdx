...
For authenticated requests, supply a Cloudflare Access or Cognito JWT in the `Authorization` header. Without it the request is anonymous and only manifests with `auth.inbound.allow_anonymous: true` will accept it. See [auth.md](/internals/auth/) for verifier configuration.
