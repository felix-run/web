...
This is the operator's reference for what Felix emits, what it means, and what to alert on. It complements [governance.md](/internals/governance/) (which covers *why* a wrapper fires) by focusing on the *signals* the wrapper leaves behind.
...
- **Counters / histograms** — `recordCounter(name, labels, value)` / `recordHistogram(name, value, labels)` in [`src/observability/metrics.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/observability/metrics.ts). In dev/tests these log structured JSON lines so `wrangler tail` shows them; in production they fan out via `env.METRICS.writeDataPoint({ blobs: [...labels], doubles: [value] })` to Workers Analytics Engine (dataset declared in `wrangler.jsonc` as `analytics_engine_datasets`). When the binding is absent the stdout fallback continues to fire so dev parity holds.
- **Spans** — `withSpan(name, fn, attrs)` and `manifestSpan(name, version)` in [`src/observability/tracing.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/observability/tracing.ts). Spans wrap each tool dispatch and the outer `buildAgent`/`invoke` boundary, attaching `duration_ms`, `transport`, `manifest_id`, `status`, and on error `error_code`. Spans are kept in-process today (the export sink is a no-op); pair them with `wrangler tail` to see the structured span lines.
- **Audit events** — `recordEvent(AuditEvent)` in [`src/audit/store.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/audit/store.ts) writes to the `AUDIT_QUEUE` queue. The queue consumer batches up to 50 events per pull and inserts into Postgres (`audit_events` table, composite PK `(tenant_id, id)`) in one multi-row INSERT. Failed batch inserts fall back to per-row inserts so one poison row doesn't starve the queue.
...
|---|---|---|---|
| `orchestrator_tool_calls` | `manifest_id`, `transport`, `status`, `error_code?` | [`patterns/react.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/patterns/react.ts) | One per tool dispatch. `transport ∈ {local, mcp, a2a, container, queue, sandbox, browser, unknown}`. `status ∈ {ok, error}`. On `error`, `error_code` is one of `ToolErrorCode`. |
| `orchestrator_tokens` | `manifest_id`, `model`, `kind` | [`patterns/model.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/patterns/model.ts) | Token usage per model call. `kind ∈ {input, output, cache_creation, cache_read}`. |
| `orchestrator_policy_decisions` | `outcome`, `policy_id`, `manifest_id`, `transport` | [`policy/wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/policy/wrap.ts) | Only `outcome: 'denied'` is emitted — allows are quiet. The `transport` label is the inner tool's transport (preserved through the wrapper). |
| `orchestrator_command_screened` | `outcome`, `manifest_id`, `transport` | [`policy/command-wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/policy/command-wrap.ts) | Only non-allow outcomes are emitted — clean commands are quiet. `outcome ∈ {denied, approval_required, approved}`. |
| `orchestrator_command_screen_no_context` | `tool`, `manifest_id` | [`policy/command-wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/policy/command-wrap.ts) | Fail-closed guard fired: a screened tool was called with no `RequestContext`, so the call was denied rather than run unscreened. Should be zero in production — a non-zero rate means an invoker is missing `runWithContext`. |
| `orchestrator_queue_token_rejected` | `reason` | [`api/internal.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/api/internal.ts) | A queue write-back presented a capability token that didn't authorize the request. `reason ∈ {malformed, bad_signature, expired, claim_mismatch}`. `expired` is routine for a job that outran its deadline; `bad_signature` or `claim_mismatch` means a consumer is presenting a token for work it wasn't given, and is worth investigating. |
| `orchestrator_secret_masked` | `tool`, `transport`, `manifest_id` | [`security/masking-wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/security/masking-wrap.ts) | A tool returned one of this Worker's own credentials and it was stripped before the model saw it. **Should be zero** — a non-zero rate means an upstream is echoing what we sent it, or a container is printing its environment, and the credential should be rotated. |
| `orchestrator_content_screened` | `outcome`, `manifest_id`, `transport` | [`screening/wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/screening/wrap.ts) | One per screened tool result. `outcome ∈ {allowed, flagged, unavailable_closed, unavailable_open, skipped_dev}`. A rising `flagged` rate on one transport is the signal that a specific upstream is serving injected content. |
| `orchestrator_content_screen_skipped` | `reason`, `manifest_id` | [`screening/classifier.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/screening/classifier.ts) | The classifier could not run. `reason ∈ {no_ai_binding, classifier_error, too_large}`. Outside `development` this pairs with an `unavailable_closed` outcome (content denied), so a non-zero rate means screened tools are failing closed. |
| `orchestrator_job_runs` | `status`, `trigger`, `manifest_id` | [`jobs/execute.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/execute.ts) | One per executed scheduled job. `status ∈ {ok, error, skipped}`, `trigger ∈ {scheduled, manual}`. A rising `error` rate on one `manifest_id` usually means a manifest was renamed or deleted out from under a job. |
| `orchestrator_job_deferred` | `manifest_id` | [`jobs/cron.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/cron.ts) | A due job was left unclaimed because the tick hit its execution cap. Occasional is normal; sustained means more executable jobs are due each tick than one tick can run, and they will keep sliding. |
| `orchestrator_job_sweep_failures` | `step` | [`jobs/cron.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/cron.ts) | A per-job database step failed during the sweep (`step ∈ {claim, reschedule}`). The job is skipped for that tick and retried on the next one; the rest of the sweep continues. **Should be zero.** |
| `orchestrator_job_record_failures` | `manifest_id` | [`jobs/cron.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/cron.ts) | A job ran but its bookkeeping write failed, so `last_status` is stale. The run itself is still in the `job_run` audit trail. **Should be zero.** |
| `orchestrator_limit_breaches` | `limit`, `manifest_id`, `transport?` | [`limits/wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/limits/wrap.ts) | `limit ∈ {max_tool_calls, max_peer_hops, max_wall_clock_seconds, max_input_tokens, max_output_tokens}`. `transport` is attached only when known. |
| `orchestrator_guardrail_blocks` | `surface`, `manifest_id`, `transport` | [`guardrails/wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/guardrails/wrap.ts) | Emitted on every guardrail evaluation (matched or clean). `surface ∈ {input, output}`. |
| `orchestrator_memory_captured` | `manifest_id`, `count` | [`memory/capture.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/memory/capture.ts) | One per turn that captured at least one fact; `count` is how many were written. A `count` that is always at the `max_facts` ceiling suggests the extractor is over-storing. |
| `orchestrator_memory_consolidated` | `manifest_id`, `deleted` | [`memory/consolidate.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/memory/consolidate.ts) | One per consolidated pool; `deleted` is how many facts the pass removed. A pass that consistently deletes nothing usually means `after_facts` is set below the point where contradictions appear. |
| `orchestrator_memory_consolidate_errors` | `manifest_id` | [`memory/consolidate.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/memory/consolidate.ts), [`jobs/memory-consolidation.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/memory-consolidation.ts) | The reconciliation model call threw, or a pool failed outright. |
| `orchestrator_memory_consolidate_deferred` | `manifest_id` | [`jobs/memory-consolidation.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/memory-consolidation.ts) | A pool was eligible but the tick hit its per-sweep cap. Sustained means more pools need attention each tick than one tick consolidates. |
| `orchestrator_memory_capture_errors` | `manifest_id` | [`memory/capture.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/memory/capture.ts) | The extraction model call threw. Capture is fire-and-forget, so this is the only signal that memory silently stopped filling. |
| `orchestrator_judge_scores` | `judge`, `tool`, `verdict`, `manifest_id` | [`guardrails/judge-wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/guardrails/judge-wrap.ts) | One per completed LLM-judge invocation. `verdict ∈ {pass, fail}`. Disambiguate `reflect` verifier vs `guardrails.judges[]` rubric judges by `payload.source` on the paired `judge_score` audit. |
| `orchestrator_judge_skipped` | `reason`, `judge`, `manifest_id` | [`guardrails/judge-wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/guardrails/judge-wrap.ts) | A judge was configured but couldn't run (e.g. `reason: 'no_ai_binding'`). |
| `orchestrator_judge_error` | `judge`, `manifest_id` | [`guardrails/judge-wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/guardrails/judge-wrap.ts) | A judge invocation threw. |
| `orchestrator_plan_steps` | `manifest_id`, `status` | [`patterns/plan-execute.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/patterns/plan-execute.ts) | One per `plan_execute` step. `status ∈ {ok, error, replanned}`. Pair with the `plan_step` audit row to see the step id and tool-call set. |
| `orchestrator_approval_requests` | `manifest_id`, `transport` | [`approvals/wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/approvals/wrap.ts) | One per pending-approval branch. |
| `orchestrator_approval_decisions` | `outcome`, `manifest_id`, `transport` | [`approvals/wrap.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/approvals/wrap.ts) | `outcome ∈ {approved, denied, denied_unattended}`. Expirations don't emit here — they show up only as audit events. `denied_unattended` means a background run (cron / replay / detached eval) hit an approval-gated tool without `allow_unattended`; a steady rate means a scheduled job is asking for something it can never be granted, and either the job or the rule needs changing. |
| `orchestrator_checkpoint_failures` | `manifest_id` | [`session/do-session.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/session/do-session.ts) | Terminal failure after 3 retries to persist a session event batch to `ConversationDO`. **Should be zero.** |
| `orchestrator_anomalies` | `manifest_id`, `tool`, `error_code` | [`jobs/anomaly-detector.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/anomaly-detector.ts) | One per per-tool error-rate anomaly flagged against the 24h EWMA baseline during a cron sweep. |
| `orchestrator_auto_rollbacks` | `manifest_id` | [`jobs/anomaly-detector.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/anomaly-detector.ts) | A flagged anomaly belonged to a canary; its `canary_weight` was set to 0. |
| `orchestrator_durable_started` | `manifest_id` | [`manifests/builder.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/manifests/builder.ts) | A `DurableAgent` kicked off an `AGENT_WORKFLOW` run. |
| `orchestrator_durable_complete` | `manifest_id`, `status` | [`manifests/builder.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/manifests/builder.ts) | A durable workflow run finished. `status` is `complete` or the terminal workflow status. |
| `orchestrator_durable_fallback` | `manifest_id` | [`manifests/builder.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/manifests/builder.ts) | `execution.mode: durable` declared but `env.AGENT_WORKFLOW` binding absent. Fell back to transient. **Should be zero in prod.** |
| `orchestrator_model_switches` | `from`, `to`, `reason` | [`patterns/model.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/patterns/model.ts) | Fallback chain or confidence-routed escalation fired. `reason ∈ {provider_error, low_confidence}` — `provider_error` for fallback-chain switches, `low_confidence` for confidence-escalation switches. |
| `orchestrator_unhandled_error` | `path`, `method`, `tenant_id` | [`src/app.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/app.ts) | The Hono `onError` boundary caught a non-`HTTPException`. **Should be zero.** |
| `orchestrator_audit_dropped` | `manifest_id`, `event_type` | [`audit/store.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/audit/store.ts) | Events dropped after the per-request audit cap (200) was hit. Pairs with the `audit_truncated` status marker. |
| `orchestrator_audit_dlq_received` | (none; value = events drained) | [`jobs/audit-dlq.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/audit-dlq.ts) | Audit events dead-lettered off `felix-audit-<env>` after exhausting retries, drained by the `-dlq` branch of the `queue()` handler (best-effort re-persisted to Postgres). **Non-zero means the main audit consumer is failing** — investigate Postgres/Hyperdrive health. |
| `orchestrator_retention_deleted` | `table` | [`jobs/retention.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/retention.ts) | Rows/objects pruned by the `retention_sweep` cron. `table ∈ {audit_events, plans, artifacts, memory_vectors}` (`memory_vectors` only when `MEMORY_RETENTION_DAYS` is set — unset keeps memories forever). |
| `orchestrator_conversation_idle_expired` | (none) | [`memory/conversation-do.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/memory/conversation-do.ts) | A `ConversationDO` thread's storage was wiped by its idle-TTL alarm after `CONVERSATION_IDLE_TTL_DAYS` of inactivity. |
| `orchestrator_artifact_spill_failed` | `manifest_id` | [`tools/artifacts.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/tools/artifacts.ts) | An R2 artifact spill failed; the tool result was returned inline instead of as a stub. |
| `orchestrator_semantic_retrieval_failed` | `manifest_id` | [`session/semantic-strategy.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/session/semantic-strategy.ts) | The `semantic:N` strategy's BGE retrieval errored and degraded to a fallback render. |
| `orchestrator_rate_limit_binding_error` | — | [`security/rate-limit.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/security/rate-limit.ts) | The `TENANT_RATE_LIMIT` binding threw; the request was allowed through (fail-open). |
| `orchestrator_abandoned_carts_detected` | (none; value = carts detected) | [`jobs/abandoned-cart.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/abandoned-cart.ts) | Carts flagged by the abandoned-cart cron. See [the commerce docs](/commerce/). |
| `orchestrator_geo_mention` / `orchestrator_geo_rank` | `manifest_id` (brand), `engine`, `mentioned` (mention only) | [`jobs/geo-monitor.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/geo-monitor.ts) | GEO monitor: brand-mention counter + rank histogram per tracked query replay. |
| `orchestrator_continuous_eval` | `manifest_id`, `verdict` | [`jobs/continuous-eval.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/jobs/continuous-eval.ts) | Judged canary replays from the continuous-eval cron. |
...
- [`docs/internals/governance.md`](/internals/governance/) — the wrappers that emit most of the counters above
- [`docs/internals/persistence.md`](/internals/persistence/) — the session log schema (relevant for checkpoint failures)
- [`docs/guide/management-api.md`](/guide/management-api/) — `/audit` query parameters and response shape
