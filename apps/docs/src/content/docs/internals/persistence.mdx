...
The commerce layer adds ~25 tables following the same conventions, all in the same `apps/api/migrations/0001_baseline.sql` file; their shapes and semantics are documented alongside the features in [the commerce docs](https://github.com/felix-run/web/blob/main/commerce/docs/index.md). Highlights that affect persistence reasoning:
...
- `products` carries a generated `search_tsv` tsvector column (weighted title > category > description) plus a `pg_trgm` index on `title`, powering `catalog_search`'s full-text + typo-tolerant search — see [the commerce shopping docs](https://github.com/felix-run/web/blob/main/commerce/docs/shopping.md).
