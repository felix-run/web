...
The commerce layer adds ~25 tables following the same conventions, all in the same `apps/api/migrations/0001_baseline.sql` file; their shapes and semantics are documented alongside the features in [the commerce docs](/commerce/). Highlights that affect persistence reasoning:
...
- `products` carries a generated `search_tsv` tsvector column (weighted title > category > description) plus a `pg_trgm` index on `title`, powering `catalog_search`'s full-text + typo-tolerant search — see [the commerce shopping docs](/commerce/shopping/).
...
`QueueExecutor` (`transport: queue`, [`src/tools/queue-executor.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/src/tools/queue-executor.ts)) is the dispatch half of an async-tool protocol that uses the session log as the rendezvous point. The full path:
...
The protocol is pinned by [`tests/unit/queue_async_resume.test.ts`](https://github.com/blakebauman/felix-run/blob/main/packages/harness/tests/unit/queue_async_resume.test.ts).
