...
All three run before every route. Order matters: the body cap rejects oversized payloads before any parsing; auth runs before the rate limiter so it can key on the resolved tenant id.

A small set of mounts is **self-authenticating** (`selfAuthenticatingMounts` on a plugin, threaded into `authMiddleware`): the middleware still installs an anonymous `RequestContext` but skips JWT bearer parsing, because the mount enforces its own credential inside the router.
...
`src/security/rate-limit.ts`. Reads `c.env.TENANT_RATE_LIMIT` and keys on `principal.tenantId`, after consulting plugin-contributed `keyResolvers` in order. Soft-fails open if the binding is absent (unit tests, dev probes without the unsafe binding wired). Skips `/health`, `/.well-known/*`, `/docs`, `/openapi.json`.
...
Additional cron work lands in the same handler — core jobs plus every installed plugin's `cronTasks` (each isolated in its own `try/catch`):
