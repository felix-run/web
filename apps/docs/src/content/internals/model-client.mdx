...
This page describes the former TypeScript Cloudflare Workers harness (Workers AI / AI Gateway). The live harness is Python in [felix-run/felix](https://github.com/felix-run/felix). Prefer that repo for current model providers.
:::
...
Providers are looked up through a registry, not a hardcoded switch — the open registry is one of the three managed-agents seams (see [docs/README.md](/)). Each provider implements `ModelClient` and registers itself at module load:
...
URL: `${AI_GATEWAY_BASE}/${AI_GATEWAY_SLUG}/anthropic/v1/messages`. The `count_tokens` companion lives at `…/anthropic/v1/messages/count_tokens` and is used by `checkPreflightTokenBudget` (see [governance.md](/internals/governance/)).
