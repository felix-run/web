...
This page describes the former TypeScript Cloudflare Workers harness (Hyperdrive, DOs, R2). The live harness is Python in [felix-run/felix](https://github.com/felix-run/felix) — Postgres + Valkey + object store. Prefer that repo and [getting started](/guide/getting-started/) / [deploy](/guide/deploy/) for current persistence.
:::
...
`kind` values in the shared table: `fact` / `preference` / `episode` (semantic memory), and `procedural` (procedural memory — `storeProcedure` writes successful past plans tagged with the manifest; `recall_procedure` and the `plan_execute` planner few-shots read them back). All share the same 768-dim BGE embedding pipeline.

Retention: OPT-IN via `MEMORY_RETENTION_DAYS` — unset (the default) keeps memories forever; long-term memory is state, not a log. When set, the `retention_sweep` cron prunes rows older than the window, bounded per tick like the other tables.
...
`QueueExecutor` (`transport: queue`, [`src/tools/queue-executor.ts`](https://github.com/felix-run/web/blob/main/packages/src/tools/queue-executor.ts)) is the dispatch half of an async-tool protocol that uses the session log as the rendezvous point. The full path:
...
The protocol is pinned by [`tests/unit/queue_async_resume.test.ts`](https://github.com/felix-run/web/blob/main/packages/tests/unit/queue_async_resume.test.ts).

## Durable Objects

See [architecture.md](/internals/architecture/) for the inventory. Each DO is its own piece of state:
