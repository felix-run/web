...
This page describes the former TypeScript Cloudflare Workers harness. The live harness is Python in [felix-run/felix](https://github.com/felix-run/felix). Prefer that repo and [getting started](/guide/getting-started/) / [deploy](/guide/deploy/) for current runtime behavior.
:::
...
`buildAgent` step 10 ([src/manifests/builder.ts](https://github.com/felix-run/web/blob/main/packages/src/manifests/builder.ts)):
...
Every wrapper returns its deny via `denyOutput(content, source)` from [src/tools/types.ts](https://github.com/felix-run/web/blob/main/packages/src/tools/types.ts) — they never throw. The model sees the `content` string in the tool result and can adapt: retry with different args, try a different tool, or surface the limitation to the user. Throwing would abort the loop and lose context.
...
Without `signal`, the wall-clock check only blocks the *next* tool call — a long-running fetch already in-flight runs to completion. With it, AbortError surfaces and the tool returns the catch-block output (e.g. `[cancelled] ...`). The built-in non-local executors — `A2AExecutor` ([src/a2a/client.ts](https://github.com/felix-run/web/blob/main/packages/src/a2a/client.ts)), `McpExecutor` ([src/mcp/client.ts](https://github.com/felix-run/web/blob/main/packages/src/mcp/client.ts)), and `ContainerExecutor` ([src/tools/container-executor.ts](https://github.com/felix-run/web/blob/main/packages/src/tools/container-executor.ts)) — already plumb the signal through to their outbound fetches; custom tools and custom executors that fetch external services should match the pattern.
...
Absolute ceilings (`ABSOLUTE_LIMITS` in [src/limits/models.ts](https://github.com/felix-run/web/blob/main/packages/src/limits/models.ts)) apply even if the schema is bypassed — `applyLimits` and `checkTokenBudget` run every declared cap through `clampLimits` before enforcing, so a `Limits` object built without Zod (a test harness or future programmatic builder) can't exceed the ceiling. `null` caps stay `null` (no manifest-level cap):
...
Tool wrappers read it through `currentLimitState()` ([src/limits/state.ts](https://github.com/felix-run/web/blob/main/packages/src/limits/state.ts)). Because it's AsyncLocalStorage-bound, no parameter threading is needed and tool authors cannot tamper with it.
...
Call sites in [src/limits/wrap.ts](https://github.com/felix-run/web/blob/main/packages/src/limits/wrap.ts):
