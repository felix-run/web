...
This page describes the former TypeScript Cloudflare Workers harness. The live harness is Python in [felix-run/felix](https://github.com/felix-run/felix). Prefer that repo and [getting started](/guide/getting-started/) / [deploy](/guide/deploy/) for current auth modes (`jwt` / `api_key` / `none`).
:::
...
`selfAuthenticatingMounts` (contributed by plugins and threaded into `authMiddleware` by `createApp`) tells `authMiddleware` to skip JWT bearer parsing for those path prefixes — the mount enforces its own credential inside the router. `/internal/*` uses the `x-consumer-token` / `x-consumer-secret` header against `CONSUMER_SHARED_SECRET`. All shared-secret compares go through `src/security/constant-time.ts`.
