...
This page describes the former TypeScript Cloudflare Workers harness test layout (vitest + miniflare). The live harness is Python in [felix-run/felix](https://github.com/felix-run/felix). Prefer that repo for current test commands.
:::
...
Beyond the core bindings, the miniflare block also declares the `JOBS_QUEUE` producer (`felix-jobs`, the `transport: queue` tool seam) and plain-var test credentials such as `JWKS_PUBLIC` so auth-gated integration tests run without real secrets.
